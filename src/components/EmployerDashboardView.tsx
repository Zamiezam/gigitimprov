    ...
    try {
      ...
      if (!error && data && data.length > 0) {
        const mappedApplicants: Applicant[] = data.map((app: any) => ({
          ...
          avatar: app.profiles?.avatar_url || `https://ui-avatars.com/api/?name=${encodeURIComponent(app.profiles?.full_name || 'Worker')}&background=0D8ABC&color=fff`,
                  ...
                  <button 
                    onClick={() => {
                      ...
                      try {
                        ...
                        autoTable(doc, {
                          startY: 110,
                          head: [['SDG Goal', 'Impact Metric', 'Value']],
                          body: [
                            ['SDG 1: No Poverty', 'B40 Student Wages Distributed', `RM ${esgStats.b40WagesPaid.toLocaleString()}`],
                            ['SDG 8: Decent Work & Economic Growth', 'Total Wages Paid', `RM ${esgStats.totalWagesPaid.toLocaleString()}`],
                            ['SDG 4: Quality Education', 'Total Training Hours Provided', `${esgStats.totalHoursWorked} Hours`],
                            ['SDG 8: Decent Work & Economic Growth', 'Unique Students Hired', `${esgStats.studentsHired} Students`],
                            ['SDG 4: Quality Education', 'Top Mentored Skill', `${esgStats.topSkillMentored}`],
                            ['SDG 10: Reduced Inequalities', 'Average Worker Rating', `${esgStats.avgRating > 0 ? esgStats.avgRating + ' / 5.0' : 'N/A'}`],
                            ['SDG 8: Decent Work & Economic Growth', 'Opportunities Created (Gigs)', `${esgStats.totalGigsPosted}`],
                          ],
